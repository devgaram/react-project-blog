...
export function* requestLogin() {
  while (true) {
    ...
    const { result, error } = yield call(Auth.login, payload.userid, payload.password); // 서버에 로그인 요청
    // 로그인 에러 발생
    if (error != null) {
      if (error.response.data != null) {
        yield put({ type: FAILURE_LOGIN, payload: error.response.data.message });
      } else {
        yield put({ type: FAILURE_LOGIN, payload: '서버 에러로 로그인 실패했습니다.' });
      }
       continue;
    }
