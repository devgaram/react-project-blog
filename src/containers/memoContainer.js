...
const MemoContainer = () => {
  ...
  const [ tagArray, tagArrayDispatch ] = useReducer((tagArray, { type, value }) => {
    switch (type) {
      case "add":    
  ...
  useEffect(() => {
    ...
    let [ memoLabelContent, memoContent ] = document.getElementsByClassName("memo-content");
    let [ memoLabelTitle, memoTitle ] = document.getElementsByClassName("memo-title");
    let memoToolGroup = document.getElementsByClassName("memo-tool-group")[0];
    let [ iconBg, iconTag, iconCheckbox ] = document.getElementsByClassName("memo-icons");
    let memoIconDetail = document.getElementsByClassName("icon-detail")[0];
    let bgColorsDetail= document.getElementsByClassName("bg-colors-detail")[0].getElementsByTagName("div");
    ...
    const memoListener = event => {
      if (event.target !== memoClose && memoContainer.contains(event.target)) {
        if ((event.target === memoContent || event.target === memoLabelContent) && memoTitle.getAttribute("hidden") !== null) {
        ...
        if (event.target.nextElementSibling === memoContent || event.target.nextElementSibling === memoTitle) {
